...
export function Navbar({ activeSection, visible = true }) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Hide if we are within 300px of the bottom of the page (where Contact section is)
      const scrolledToBottom = window.innerHeight + window.scrollY >= document.body.offsetHeight - 300
      setIsAtBottom(scrolledToBottom)
      // Shrink navbar as soon as the user scrolls down
          ...
          {navItems.map((item) => {
            ...
            return (
              <a
                key={item.id}
                href={`#${item.id}`}
                className="relative px-5 py-2 font-label-mono text-xs tracking-widest transition-colors duration-300"
