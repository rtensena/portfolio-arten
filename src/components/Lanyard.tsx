...
function CameraPositioner() {
  ...
  useEffect(() => {
    // 70% from the left edge = 20% to the right of the center
    // So the camera needs to be shifted 20% to the left
    camera.position.x = -(viewport.width * 0.2);
...
function Band({
  ...
  const segmentProps: RigidBodyProps = {
    type: 'dynamic',
    canSleep: false, // Prevent sleeping to allow idle swing
    colliders: false,
    angularDamping: 4,
    linearDamping: 4
  };
  ...
  const cardMap = useMemo(() => {
    ...
    const drawFitted = (img: any, rect: typeof FRONT_UV_RECT) => {
      ...
      // Draw white outer frame
      ctx.lineWidth = W * 0.04; // Proportional border width
  ...
  useEffect(() => {
    // Initial swing/impulse when component mounts
    const timer = setTimeout(() => {
      card.current?.applyImpulse({ x: 0.5, y: 0, z: 1.5 }, true);
    }, 800);
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ...
      // Add gentle idle breeze if not dragged
      if (!dragged) {
        const t = state.clock.elapsedTime;
        card.current.applyImpulse({ x: Math.sin(t * 0.8) * 0.005, y: 0, z: Math.cos(t * 0.5) * 0.005 }, true);
      }
    }
  });
